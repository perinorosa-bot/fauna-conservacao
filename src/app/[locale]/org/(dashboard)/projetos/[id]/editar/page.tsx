...
export default function EditarProjetoPage() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      setSpecies(project.species ?? '')
      setBiome(project.biome ?? '')
      setCountry(project.country ?? 'Brasil')
      setCoverUrl(project.cover_image_url ?? '')
      setVideoLinks(project.video_links?.length ? project.video_links : [''])
  ...
  async function handleImageUpload(file: File) {
    ...
    setUploading(false)
    if (upErr) {
      setError(o('uploadErrorPrefix') + upErr.message)
      return
    }
  ...
  async function handleDelete() {
    if (!confirm(o('confirmDeleteProjectLong'))) return
    setDeleting(true)
    const { error: delErr } = await supabase.from('projects').delete().eq('id', params.id)
    setDeleting(false)
  ...
  if (loading) {
    return <div className="text-cream/40 text-sm">{o('loading')}</div>
  }
  if (notFound) {
    return (
      <div className="max-w-2xl">
        <p className="text-cream/60">{o('projectNotFound')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-10">
        <h1 className="font-serif text-4xl font-light text-cream mb-2">{o('editProjectTitle')}</h1>
        <p className="text-cream/40 text-sm">{o('editProjectSubtitle')}</p>
      </div>

      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('projectTitle')} {o('required')}</label>
          <input className={inputClass} type="text" value={title}
                 onChange={e => { setTitle(e.target.value); if (!slug) setSlug(slugify(e.target.value)) }} required />
        </div>

        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">
            {o('urlSlugPrefix')}<span className="text-sage">{slug || o('urlSlugFallback')}</span>
          </label>
          <input className={inputClass} type="text" value={slug}
                 onChange={e => setSlug(slugify(e.target.value))} required />
        </div>

        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('shortDesc')} {o('required')}</label>
          <textarea className={`${inputClass} resize-none`} rows={2}
                    value={description} onChange={e => setDescription(e.target.value)} required />
        </div>

        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('fullDesc')}</label>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input className={inputClass} type="text" value={species} onChange={e => setSpecies(e.target.value)} />
          </div>
          <div>
            <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('biome')}</label>
            <select className={inputClass} value={biome} onChange={e => setBiome(e.target.value)}>
              <option value="">—</option>
              {BIOMES.map(b => <option key={b} value={b}>{b}</option>)}
            </select>
          </div>
          <div>
            <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('countryLabel')}</label>
          ...
          <div className="flex gap-2 mb-2">
            <button type="button" onClick={() => setCoverTab('url')}
                    className={`text-[10px] tracking-widest uppercase px-3 py-1.5 rounded-sm transition-colors ${
                      coverTab === 'url' ? 'bg-sage/20 text-sage' : 'text-cream/30 hover:text-cream'}`}>URL</button>
            <button type="button" onClick={() => setCoverTab('upload')}
                    className={`text-[10px] tracking-widest uppercase px-3 py-1.5 rounded-sm transition-colors ${
                      coverTab === 'upload' ? 'bg-sage/20 text-sage' : 'text-cream/30 hover:text-cream'}`}>Upload</button>
          </div>
          {coverTab === 'url' ? (
            <input className={inputClass} type="url" value={coverUrl} onChange={e => setCoverUrl(e.target.value)} />
          ) : (
            <div>
              ...
              {coverUrl && !uploading && <p className="text-sage text-[10px] mt-1.5 truncate">{coverUrl}</p>}
            </div>
          )}
        </div>
          ...
          <div className="flex flex-col gap-2">
            {videoLinks.map((v, i) => (
              <div key={i} className="flex gap-2">
                <input className={inputClass} type="url" value={v}
        ...
        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('tags')}</label>
          <input className={inputClass} type="text" value={tagsInput} onChange={e => setTagsInput(e.target.value)} />
        </div>

        <div>
          <label className="text-cream/50 text-xs tracking-wide block mb-1.5">{o('statusLabel')}</label>
          <select className={inputClass} value={status} onChange={e => setStatus(e.target.value as any)}>
            <option value="active">{o('statusActive')}</option>
            <option value="paused">{o('statusPaused')}</option>
            <option value="completed">{o('statusCompleted')}</option>
          </select>
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button type="button" onClick={handleDelete} disabled={deleting || submitting}
            ...
            {deleting ? o('deletingBtn') : o('deleteProjectBtn')}
          </button>
        </div>
      </form>
    </div>
