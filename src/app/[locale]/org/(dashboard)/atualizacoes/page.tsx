...
export default function OrgUpdatesListPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm(o('confirmDeleteUpdate'))) return
    setDeletingId(id)
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="font-serif text-4xl font-light text-cream mb-2">{o('updates')}</h1>
          <p className="text-cream/40 text-sm">{o('updatesPublished', { count: updates.length })}</p>
        </div>
      ...
      {updates.length === 0 ? (
        <div className="bg-canopy/20 border border-white/[0.06] rounded-xl p-10 text-center">
          <p className="text-cream/30 text-sm mb-4">{o('noUpdatesYet')}</p>
            ...
            <div key={u.id} className="bg-canopy/30 border border-white/[0.06] rounded-xl px-6 py-4 flex items-center gap-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-cream/40 text-xs mt-0.5 truncate">
                  {u.project?.title ?? '—'} · {u.author_name} · {format.dateTime(new Date(u.created_at), { dateStyle: 'short' })}
                </p>
              </div>
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <button onClick={() => handleDelete(u.id)} disabled={deletingId === u.id}
                  ...
                  {deletingId === u.id ? o('deletingBtn') : o('deleteBtn')}
                </button>
              </div>
            </div>
          ))}
